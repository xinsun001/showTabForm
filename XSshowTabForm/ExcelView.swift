...
@objc public protocol ExcelViewDelegate: NSObjectProtocol {
    @objc optional func excelView(_ excelView: ExcelView, didTapGridWith content: String, indexRows: Int, index: Int)
...
open class ExcelView: UIView {
    ...
    func setupViews() {
        ...
        leftHeaderTableView.separatorStyle = .singleLine
        leftHeaderTableView.separatorColor = UIColor.lightGray
        leftHeaderTableView.separatorInset = UIEdgeInsets(top: 0, left: 0, bottom: 0, right: 0)
...
extension ExcelView: UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if tableView == leftHeaderTableView {
            ...
            contentCell.didTapClosure = {[weak self] (itemContent, index) in
                self?.delegate?.excelView?(self!, didTapGridWith: itemContent, indexRows: indexPath.row, index: index)
            }
        }else {
...
class ContentCell: UITableViewCell {
    ...
    @objc func didTap(_ tap: UITapGestureRecognizer) {
        ...
        for (index, label) in itemLabels.enumerated() {
            if label.frame.contains(point) {
                didTapClosure?(rowDatas![index], index)
                break
            }
        }
    }
